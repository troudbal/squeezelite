...
static void pulse_set_volume(struct pulse *p, unsigned left, unsigned right) {
	...
	if (op != NULL) {
		...
		if (loglevel >= lDEBUG) {
			char s[20];
			LOG_SQ_DEBUG("sink input volume set to %s", pa_cvolume_snprint(s, sizeof(s), &volume));
		}
		pa_operation_unref(op);
	}
}
...
void set_sample_rate(uint32_t sample_rate) {
	...
	if (op != NULL) {
		if (loglevel >= lDEBUG) {
			LOG_SQ_DEBUG("stream sample rate set to %d Hz", sample_rate);
		}
		pa_operation_unref(op);
	}
	else {
		...
		if (output_off) {
...
#endif
				LOG_SQ_DEBUG("destroying PulseAudio playback stream");
			...
			if (pulse.stream == NULL) {
				...
				if (pulse_stream_create(&pulse)) {
					LOG_SQ_DEBUG("PulseAudio playback stream on sink %s open", pulse.sink_name);
