	...
	while (n) {
		...
		case STREAM_OGG_SYNC:
			...
			if (ogg.match == 4) {
				...
				if (!ogg.match) LOG_SQ_INFO("no OggS at expected position %zu/%zu", pos, n);
				return;
			}
			break;
		...
		case STREAM_OGG_PAGE: {
			...
			if (ofs) {
				...
				wake_controller();
				LOG_SQ_INFO("metadata length: %u", stream.header_len - 3);
			}
		...
		if (_poll(&pollinfo, 100)) {
			...
			if (pollinfo.revents & (POLLIN | POLLHUP)) {
				...
				if (stream.state == RECV_HEADERS) {
					...
					if (n <= 0) {
						...
						if (!ssl && !stream.header_len) {
							...
							LOG_SQ_INFO("now attempting with SSL");
				...
				// stream body into streambuf
					...
					if (n == 0) {
						LOG_SQ_INFO("end of stream (%u bytes)", stream.bytes);
						_disconnect(DISCONNECT, DISCONNECT_OK);
					}
					if (n < 0) {
						error = _last_error();
						if (error != ERROR_WOULDBLOCK) {
							LOG_SQ_INFO("error reading: %s (%d)", strerror(error), error);
							_disconnect(DISCONNECT, REMOTE_DISCONNECT);
						}
					}
...
void stream_init(log_level level, unsigned stream_buf_size) {
	...
	if (!ogg.dl.handle) {
		LOG_SQ_INFO("ogg dlerror: %s", dlerror());
	}
