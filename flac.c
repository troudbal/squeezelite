...
static void metadata_cb(const FLAC__StreamDecoder* decoder, const FLAC__StreamMetadata* metadata, void* client_data) {
	...
	case FLAC__METADATA_TYPE_STREAMINFO:
		LOG_SQ_INFO("stream parameters rate:%d, channels:%d, size:%d", metadata->data.stream_info.sample_rate, 
	...
	case FLAC__METADATA_TYPE_VORBIS_COMMENT: {
		...
		for (int i = 0; i < metadata->data.vorbis_comment.num_comments; i++, comment++) {
			LOG_SQ_INFO("stream metadata %*s", comment->length, comment->entry);
		}
	}
	default:
		break;
...
static void flac_open(u8_t sample_size, u8_t sample_rate, u8_t channels, u8_t endianness) {
	...
	if ( f->container == 'o' ) {
		LOG_SQ_INFO("ogg/flac container - using init_ogg_stream");
...
#endif
		LOG_SQ_INFO("using chained stream decoding");
#else
...
static decode_state flac_decode(void) {
	...
	if (state == FLAC__STREAM_DECODER_END_OF_LINK) {
		if (!FLAC(f, stream_decoder_finish_link, f->decoder)) {
			LOG_SQ_INFO("flac error: could not finish chained link");
			return DECODE_ERROR;
		}
...
static bool load_flac() {
	...
	LOG_SQ_INFO("loaded %s", name);
#elif FLAC_API_VERSION_CURRENT < 14
	LOG_SQ_INFO("OggFlac chaining disabled");
#endif
...
struct codec *register_flac(void) {
	...
	LOG_SQ_INFO("using flac to decode ogf,flc");
	return &ret;
}
