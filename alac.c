...
static int read_mp4_header(void) {
	...
	while (bytes >= 8) {
		...
		if (!strcmp(type, "alac") && bytes > len) {
			...
			LOG_SQ_INFO("allocated write buffer of %u bytes", block_size);
			if (!l->writebuf) {
		...
		if (!strcmp(type, "stsz") && bytes > len) {
			...
			if (!l->default_block_size) {
				...
				LOG_SQ_DEBUG("total blocksize contained in stsz %u", entries);
			} else {
				LOG_SQ_DEBUG("fixed blocksize in stsz %u", l->default_block_size);
            }
		}
		...
		if (!strcmp(type, "stts") && bytes > len) {
			...
			LOG_SQ_DEBUG("total number of samples contained in stts: " FMT_u64, l->sttssamples);
		}
		...
		if (!strcmp(type, "mdat")) {
			...
			bytes  -= 8;
			if (l->play) {
				LOG_SQ_DEBUG("type: mdat len: %u pos: %u", len, l->pos);
...
static decode_state alac_decode(void) {
	...
	if (l->consume) {
		...
		LOG_SQ_DEBUG("consume: %u of %u", consume, l->consume);
		_buf_inc_readp(streambuf, consume);
		l->pos += consume;
		l->consume -= consume;
		UNLOCK_S;
	...
	if (l->samples) {
		if (l->samples < frames) {
			LOG_SQ_DEBUG("gapless: trimming %u frames from end", frames - l->samples);
...
struct codec *register_alac(void) {
	...
	LOG_SQ_INFO("using alac to decode alc");
	return &ret;
}
